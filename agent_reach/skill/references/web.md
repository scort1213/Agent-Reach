...
列表发现使用当前宿主提供的 Computer Use，可通过原生 Chrome 窗口读取和点击，也可使用可用的网页接口；记录实际入口。网页接口初始化失败只阻断该入口，不能直接判定原生窗口失败。先核对目标既有拒绝，无恢复证据不切换入口绕行。不另启 Playwright/读取隐藏应用状态，不声称独立 CLI 可以搜索。
