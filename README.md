...
|------|---------|-----------|-------|
| 🌐 **网页** | 阅读任意网页 | — | 无需配置 |
| 📰 **今日头条（本 Fork）** | 指定公开文章、已有清单批量读取 | 宿主浏览器辅助关键词和账号发现 | 告诉 Agent「搜索关键词/读取账号的前5篇图文」；不含视频与评论 |
...
输出标题、来源、发布时间（取得时）、正文、图片引用、原链接和获取时间。读取器先尝试
移动文章数据，普通网络或解析失败时才尝试 Jina；访问限制、验证或限流会停止，不切换
服务绕过。Jina 可能使用缓存，输出会说明这一点。失败返回非零退出码；JSON 错误带原因。
`doctor` 只报告读取器存在和“未联网验证”，不会把安装成功当成文章必定可读。
具体边界见 [头条使用说明](agent_reach/guides/setup-toutiao.md)。

关键词和账号列表可由有浏览器能力的 Agent 自动发现，再连续读取正文：

```bash
agent-reach read-toutiao-batch candidates.json --output ./toutiao-run --limit 5
```

自然语言例子：「搜索人工智能最近24小时的5篇头条文章并分析」；「分析中国网信杂志前5篇文章」。
关键词默认24小时，账号按展示顺序；逐篇保存正文和失败原因，支持原清单续跑。
发现列表依赖宿主浏览器工具，独立命令行没有关键词搜索接口；首批处理成功不代表抓全历史。
